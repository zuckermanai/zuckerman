...
- Each memory should be distinct and non-redundant`;
  ...
  /**
   * Convenience method for handling new messages
   */
  async onNewMessage(userMessage: string, metadata?: MemoryMetadata): Promise<void> {
    await this.classifyAndSave({
      type: "message",
      content: userMessage,
      metadata,
    });
   ...
   */
  async onAction(action: string, metadata?: MemoryMetadata): Promise<void> {
    await this.classifyAndSave({
      type: "action",
      content: action,
      metadata,
   ...
   */
  async onThought(thought: string, metadata?: MemoryMetadata): Promise<void> {
    await this.classifyAndSave({
      type: "thought",
      content: thought,
      metadata,
   ...
   */
  async onDecision(decision: string, metadata?: MemoryMetadata): Promise<void> {
    await this.classifyAndSave({
      type: "decision",
      content: decision,
      metadata,
  ...
  /**
   * Handle experience events
   */
  async onExperience(experience: string, metadata?: MemoryMetadata): Promise<void> {
    await this.classifyAndSave({
      type: "experience",
      content: experience,
      metadata,
