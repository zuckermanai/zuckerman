...
export class ConversationManager {
  ...
  private async pruneUnusedMessages(
    ...
    if (!activeMessages.length || this.getTokens(activeMessages) <= threshold) return messages;

    console.log(`[ConversationManager] Pruning messages:`, messages.length, this.getTokens(messages), threshold);
    ...
    try {
      ...
      const res = await llm.call({
        messages: [{
          role: "user",
          content: `You are analyzing a conversation with ${activeMessages.length} messages (${this.getTokens(activeMessages)} tokens). The token limit is ${threshold}. Identify message indices to ignore that don't add value for future responses.
...
${text}`,
      ...
      if (match) {
        ...
        console.log(`[ConversationManager] Pruned messages:`, indices);
      ...
      console.warn(`[ConversationManager] Prune failed:`, e);
    }
