 ...
 * Get container state
 */
export async function getContainerState(containerName) {
    try {
        const { stdout } = await execDocker(["inspect", "--format", "{{.State.Running}} {{.Id}}", containerName], { allowFailure: false });
 ...
 */
export async function execInContainer(containerName, command, args = [], options) {
