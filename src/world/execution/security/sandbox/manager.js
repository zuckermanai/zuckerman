import { createSandboxContainer, getContainerState, ensureContainerRunning, removeContainer, execInContainer, isDockerAvailable, } from "./docker.js";
import { getRegistryEntry, updateRegistryEntry, removeRegistryEntry, } from "./registry.js";
 ...
 */
function resolveScopeKey(scope, sessionId, agentId) {
    if (scope === "shared") {
        return "shared";
    }
    if (scope === "per-agent") {
 ...
 */
function generateContainerName(prefix, scopeKey) {
 ...
 */
export async function executeInSandbox(sandbox, command, args = [], options) {
 ...
 */
export async function cleanupSandbox(containerName) {
    await removeContainer(containerName);
    await removeRegistryEntry(containerName);
}
